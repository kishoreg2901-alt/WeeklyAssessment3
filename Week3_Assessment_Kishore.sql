...
-- A3. You are designing a table to store product prices for an e-commerce site (e.g. ₹299.50, 
-- ₹79,999.00). Which data type is the most appropriate?
-- solution: b

-- A4. You try to INSERT a row into an orders table with a customer_id that doesn't exist in the 
-- customers table. There is a foreign key constraint. What happens?
-- solution: c
...
-- A8. Which command permanently removes an entire table (both structure and data), and 
...
-- Prediction: product_id - 3,6,8,10,11,12,13,15,17,28,29
...
GROUP BY category
HAVING COUNT(*) > 4;
-- Prediction: Electronics : 7, Apparel: 6 , Home : 5
...
SELECT product_name,
 ...
 -- Prediction: Nykaa Matte Lipstick - Mid, Lakme Eye Liner - Budget,Mamaearth Face Wash-Budget,WOW Skin Vitamin C Serum - Mid
 
 -- B8. What does this query return?
SELECT product_name, COALESCE(avg_rating, 0) AS rating
FROM products
WHERE stock_quantity = 0;
-- Prediction : JBL Flip 6 Speaker - 4.30,WOW Skin Vitamin C Serum - 4.50
...
-- C4. Write a query to display the top 5 most expensive products in the Electronics category 
...
-- C5. Write a query to list all products in either Electronics or Apparel category. Use the IN 
-- operator.
SELECT *
FROM products
...
-- C15. Write a query to show the average price per category, sorted from highest average to 
-- lowest.
SELECT category,
...
-- C16. Write a query to show the count of products and the average price for each brand, 
 -- showing only brands that have more than 1 product.
SELECT brand,
       COUNT(*) AS total_products,
       AVG(price) AS average_price
FROM products
...
-- C21. Write a query to show product_name and rating for all products. If avg_rating is NULL, 
-- display the text 'New Launch' instead
SELECT product_name,
       IFNULL(avg_rating,'New Launch') AS rating
FROM products;

-- C7 - CASE WHEN
-- C22. Write a query to display each product with a price_tier column classified as 'Budget' if 
-- price < ₹1,000, 'Mid' if price < ₹10,000, or 'Premium' if price >= ₹10,000.
SELECT product_name,
       CASE
...
-- C23. For each category, write a query that shows the total product count and the count of 
-- 'Premium' products (price >= ₹10,000). Use SUM(CASE WHEN...)
SELECT category,
       COUNT(*) AS total_products,
       SUM(CASE
...
-- column ('Cheap' if avg < ₹1500, 'Standard' if avg < ₹10000, 'Luxury' if avg >= ₹10000). Sort by 
